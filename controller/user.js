  ...
  try {
    ...
    if (firstName && lastName && email && phone && paymentMethod && pet_id && petType && petName && petAge && petBreed && petPrice) {
      const order = new Order({
        ...
        paymentMethod, paymentStatus, transactionId, pet_id, petName, petType,
        petAge, petBreed, petPrice
      });
    ...
    doc.moveDown();
    doc.lineWidth(0.5).strokeColor('#000000').moveTo(50, doc.y).lineTo(550, doc.y).stroke();
    doc.moveDown(1);
    ...
    // Customer Info
    doc.fontSize(12).text(`Customer Name: ${firstName} ${lastName}`);
